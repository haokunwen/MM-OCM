...
def create_model_and_optimizer(all_texts):
    ...
    img_model_ = img_model.Image_net(texts_to_build_vocab=all_texts,embedding_size=args.img_dim, outfit_threshold = args.max_outfit).cuda()
    ...
    text_model_ = text_model.Text_net(texts_to_build_vocab=all_texts, embedding_size=args.img_dim, outfit_threshold = args.max_outfit).cuda()
...
def train(img_model, text_model, img_optimizer, text_optimizer, dataloader, scaler):
    ...
    with tqdm(total = 2*len(dataloader)) as t:
        for i,data in enumerate(dataloader):
            ...
            with autocast():
                img_predicted_score, loss_consist, loss_ortho = img_model(img, text)
                text_predicted_score = text_model(text, img)[0].detach()
            ...
            with autocast():
                text_predicted_score, loss_consist, loss_ortho = text_model(text, img)
                img_predicted_score = img_model(img, text)[0].detach()
...
def test(img_model, text_model, dataloader):
    ...
    avg_auc, avg_acc = compute_auc_acc((img_all_predicted_socre+text_all_predicted_score)/2.0, all_labels)
...
def train_and_evaluate(img_model, text_model, img_optimizer, text_optimizer, trainset, validset, testset):
    ...
    for epoch in range(args.epoch_num):
        ...
        valid_avg_auc, valid_avg_acc = test(img_model,text_model, validloader)
        ...
        if valid_avg_auc > best_avg_score:
            ...
            test_avg_auc, test_avg_acc = test(img_model,text_model, testloader)

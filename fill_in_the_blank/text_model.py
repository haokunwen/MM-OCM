...
class TextLSTMModel(torch.nn.Module):
    def __init__(self, texts_to_build_vocab=None, word_embed_dim=512, lstm_hidden_dim=256):
        super().__init__()
...
class Text_net(nn.Module):
    def __init__(self, texts_to_build_vocab, embedding_size=256):
        super().__init__()
    ...
    def extract_text_feature(self, texts_origin):
        texts = copy.deepcopy(texts_origin)
        text_feature = self.text_backbone(texts)
        return text_feature 

    def extract_img_feature(self, img_origin):
    ...
    def extract_answers_feature(self, img_origin):
        img = copy.deepcopy(img_origin)
        batch_size = len(img)
        y = [torch.stack(i[1:]) for i in img]
        y = torch.stack(y) # batch_size, 3, 3, 224, 224
        y = y.cuda().permute(1,0,2,3,4).contiguous()  # 3, batch_size, 3, 224, 224
        feature = torch.zeros((3, batch_size, self.hidden_dim)).cuda()
        for index in range(3):
            feature[index] = self.backbone(y[index])

        return feature   # 3, batch_size, dim

    def compute_complementary_feature(self, img_feature, text_feature, adj):
        batch_size, outfitnum, dim = img_feature.size()
        adj_mask = torch.sum(adj,dim=1)   # batch_size, outfitnum  
        ...
        return loss_consist, loss_ortho, ortho_img
    ...
    def _get_outfit_graph_feat(self, adj, img_embedding, masked_embedding, mask):
        ...
        adj = torch.sum(adj_origin,dim=1)   # batch_size, outfitnum  
    ...
    def _compatibility_score(self, ilatents, adj):
        ...
        adj = torch.sum(adj,dim=1)   # batch_size, outfitnum 
    ...
    def forward(self, question_img_origin, question_text_origin, answers_img_origin, answers_text_origin, position_origin):
        question_img = copy.deepcopy(question_img_origin)
        question_text = copy.deepcopy(question_text_origin)
        answers_img = copy.deepcopy(answers_img_origin)
        answers_text = copy.deepcopy(answers_text_origin)
        position = copy.deepcopy(position_origin)

        batch_size = len(position)
        for i in range(batch_size):
            question_img[i].insert(position[i][0], answers_img[i][0])
            question_text[i].insert(position[i][0], answers_text[i][0])
        
        img_feature, adj = self.extract_img_feature(question_img)  # batch_size, nodes, dim
        text_feature = self.extract_text_feature(question_text)

        answers_img_feature = self.extract_answers_feature(answers_img)  # 3, batch_size, dim
  
        score = torch.zeros((batch_size, 4)).cuda()
        loss_consist_sum = 0
        loss_ortho_sum = 0
        ...
        for i in range(4):
            if i == 0:
                ...
                for j in range(batch_size):
                    img[j,position[j][0],:] = answers_img_feature[i-1,j,:]
                    question_text[j][position[j][0]] = answers_text[j][i]
                    text = self.extract_text_feature(question_text)

            text_detach = text.detach()
            loss_consist, loss_ortho, ortho_img = self.compute_complementary_feature(img, text_detach, adj)
            #img = self.gamma[0] * img + self.gamma[1] * ortho_text
            img = text + ortho_img
            ...
            masked_embedding = img.unsqueeze(2)  # (batch_size, outfitnum, mask_k, dim)
 
            I = self._get_outfit_graph_feat(adj, img, masked_embedding, mask_dim)
            score_temp = self._compatibility_score(I, adj)  # batch_size,
            score[:,i] = score_temp.squeeze(1)
            loss_consist_sum += loss_consist
            loss_ortho_sum += loss_ortho
        
        return score, loss_consist_sum, loss_ortho_sum

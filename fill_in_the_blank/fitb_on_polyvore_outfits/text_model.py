...
class TextLSTMModel(torch.nn.Module):
    ...
    def forward_encoded_texts(self, texts):
        ...
        for index, data in enumerate(texts):
            ...
            adj_matrix = torch.ones(max_outfit, max_outfit) - torch.eye(int(max_outfit))
            if len(length_index) < max_outfit:
                adj_matrix[len(length_index)-max_outfit:,:] = 0
                adj_matrix[:,len(length_index)-max_outfit:] = 0
...
class Text_net(nn.Module):
    ...
    def extract_text_feature(self, texts):
        text_feature, text_adj = self.text_backbone(texts)
        return text_feature, text_adj
    ...
    def _get_outfit_graph_feat(self, adj, img_embedding, masked_embedding, mask):
        ...
        relation = torch.sum(relation, dim=2) # batch_size, outfitnum, dim 
    ...
    def forward(self, text_origin, img_origin):
        text = copy.deepcopy(text_origin)
        img = copy.deepcopy(img_origin)
        text, adj = self.extract_text_feature(text)     # img: batch_size, nodes, dim    adj: batch_size, nodes, nodes
        img = self.extract_img_feature(img)

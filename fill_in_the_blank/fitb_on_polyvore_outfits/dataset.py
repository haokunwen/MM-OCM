...
class polyvore_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        out = [{'img':[], 'text':[], 'target':[]}, {'img':[], 'text':[], 'target':[]}, {'img':[], 'text':[], 'target':[]}, {'img':[], 'text':[], 'target':[]}]

        fitb = self.fitb_outfit_list[index]
        out_temp = {'img':[], 'text':[], 'target':[]}
        ...
        for im in fitb['question']:
            ...
            out_temp['img'].append(img)
            out_temp['text'].append(self.im2desc[im])
        out_temp['target'].append(self.target[index])
        
        position = fitb['position']
        for i in range(4):
            out[i] = copy.deepcopy(out_temp)
            im = fitb['answers'][i]
            img_path = os.path.join(self.impath, '%s.jpg' % im)
            img = Image.open(img_path).convert('RGB')
            if self.transform is not None:
                img = self.transform(img)
            out[i]['img'].insert(position, img)
            out[i]['text'].insert(position, self.im2desc[im])
